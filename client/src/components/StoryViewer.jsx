...
const StoryViewer = ({ viewStory, setViewStory }) => {
    ...
    const renderContent = () => {
        switch (viewStory.media_type) {
            ...
            case 'text':
                return (
                    <div className='w-full h-full flex items-center justify-center p-6 text-white text-2xl text-center'>
                        {viewStory.content}
                    </div>
                );
    ...
    return (
        <div className='fixed inset-0 h-screen bg-black bg-opacity-90 z-110 flex items-center justify-center' style={{ backgroundColor: viewStory.media_type === "text" ? viewStory.background_color : '#000000' }}>
            ...
            {/* User info - top left */}
            <div className='absolute top-4 left-4 flex items-center space-x-3 p-2 px-4 sm:p-4 sm:px-6 backdrop-blur-2xl rounded bg-black/50'>
                <img src={viewStory.user?.profile_picture} className='size-5 sm:size-8 rounded-full object-cover border border-white' />
